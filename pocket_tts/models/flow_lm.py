...
class FlowLMModel(nn.Module):
    ...
    def dtype(self) -> torch.dtype:
        # Live dtype: reads the actual parameter precision after any
        # .to(dtype=...) / .half() / .float() conversion, so callers always
        # see what the weights run at right now — not what they were built
