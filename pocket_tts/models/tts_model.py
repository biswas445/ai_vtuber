...
class TTSModel(nn.Module):
    ...
    def _encode_audio(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        latents = encoded.to(self.flow_lm.dtype)
    ...
    def get_state_for_audio_prompt(
        ...
        if isinstance(audio_conditioning, (str, Path)) and _is_safetensors_source(
            ...
            return _import_model_state(audio_conditioning, self.device, self.flow_lm.dtype)
...
def _import_model_state(
    source: str | Path, device: torch.device, dtype: torch.dtype | None = None
) -> dict[str, dict[str, torch.Tensor]]:
    # Imported voice states ship as fp32 safetensors; when the model runs at
    # another dtype they must follow it, or the first attention op crashes on
    # a dtype mismatch. dtype=None keeps whatever the file carries.
    result = {}
