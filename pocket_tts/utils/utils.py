...
def resolve_dtype(name: str | None) -> torch.dtype | None:
    """Map a config/env string ("float32", "fp16", "bfloat16", ...) to a
    torch dtype. Empty/None returns None (keep torch's default). An unknown
    name raises instead of silently falling back — a typo must never
    quietly change model precision.
    """
    if not name:
        return None
    dtype = _DTYPE_ALIASES.get(name.strip().lower())
    if dtype is None:
        raise ValueError(
            f"unknown dtype {name!r}: expected one of float32, fp32, float16, fp16, bfloat16, bf16"
        )
    return dtype
